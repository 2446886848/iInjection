...
//                        }
                ...
                //拷贝dylib
                let astring=String().appendingFormat("scp %@ %@",yololibFile, workingDirectory)
                sc.rystemCommand(astring)
                //拷贝二进制文件
    ...
    @IBAction func yololibBrowse(_ sender: AnyObject) {
        let openDialog = NSOpenPanel()
        openDialog.canChooseFiles = true
        openDialog.canChooseDirectories = false
        openDialog.allowsMultipleSelection = false
        openDialog.allowsOtherFileTypes = false
        openDialog.allowedFileTypes = [""]
        openDialog.runModal()
        if let filename = openDialog.urls.first {
            YololibInputText.stringValue = filename.path
        }
    }

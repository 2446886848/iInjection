...
class MainView: NSView, URLSessionDataDelegate, URLSessionDelegate, URLSessionDownloadDelegate {
            ...
            for file in files {
                ...
                let appLCP   = String().appendingFormat("%@/%@",appLast,appPath)
                 ...
                 if !binaryName.isEmpty {
                    //拷贝二进制文件
                    let bstring=String().appendingFormat("scp %@/%@ %@",appBundlePath,binaryName,workingDirectory)
                    sc.rystemCommand(bstring)
                    //拷贝出dylib
                    let estring=String().appendingFormat("scp %@/%@ %@", appBundlePath,dylibComp,workingDirectory)
                    sc.rystemCommand(estring)
                    
                    let filePath = workingDirectory.stringByAppendingPathComponent(binaryName)
                    let libPath = workingDirectory.stringByAppendingPathComponent(dylibComp)
                    Yololib.injectFile(filePath, libPath: libPath)
                    
                    let dstring=String().appendingFormat("cd %@ && scp %@ %@ && rm %@ && rm %@", workingDirectory,binaryName,appLCP,binaryName,dylibComp)
                    sc.rystemCommand(dstring)
                    
                    Log.write("#############################:\(bstring,dstring,estring)");
                }
                
                //MARK: Codesigning - App
                let signingFunction1 = generateFileSignFunc(payloadDirectory, entitlementsPath: entitlementsPlist, signingCertificate: signingCertificate!)
                
                
                recursiveDirectorySearch(appBundlePath, extensions: signableExtensions, found: signingFunction1)
                signingFunction(appBundlePath)
    ...
    @IBAction func do1Browse(_ sender: AnyObject) {
        let openDialog = NSOpenPanel()
        openDialog.canChooseFiles = true
        openDialog.canChooseDirectories = false
        openDialog.allowsMultipleSelection = false
        openDialog.allowsOtherFileTypes = false
        openDialog.allowedFileTypes = ["dylib","a","framework"]
        openDialog.runModal()
        if let filename = openDialog.urls.first {
            InputText.stringValue = filename.path
        }
    }
